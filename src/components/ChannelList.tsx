      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-5 rounded-2xl bg-white/60 backdrop-blur-md border border-white/40 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          {tariff === 'free' ? (
            ...
            <span className="px-3 py-1.5 text-xs font-semibold bg-gradient-to-r from-orange-55 to-pink-55/80 text-orange-900 border border-orange-200/50 rounded-lg flex items-center gap-1.5 shadow-xs">
            ...
            >
              <div>
                <div className="flex items-center justify-between">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 ${info.bg} ${info.text}`}>
                    <SocialIcon platform={channel.platform} size={13} />
                    <span>{info.name}</span>
                  </span>
                  <button 
              ...
              <div className="mt-3.5 pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600 font-bold">
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-orange-500 animate-pulse"></span>
                  Автопостинг готов
                </span>
                <span className="text-slate-400 italic font-medium">200 ₽/мес свыше лимита</span>
              </div>
            </motion.div>
          );
        })}
            ...
            <form onSubmit={handleCreateChannel} className="space-y-4">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                  {(Object.keys(NETWORK_LABELS) as SocialNetwork[]).map((net) => {
                    ...
                    return (
                      ...
                      >
                        <SocialIcon platform={net} size={18} className="mb-1.5" />
                        <span className="text-xs font-semibold truncate w-full">{items.name}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              {(platform === 'dzen' || platform === 'setka') && (
                <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 space-y-2">
                  <p className="font-bold text-slate-700 flex items-center gap-1.5 text-xs">
                    <HelpCircle className="w-4 h-4 text-pink-500" /> Подключение {NETWORK_LABELS[platform].name} в 3 простых клика:
                  </p>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  type="submit"
                  className="px-5 py-2.5 text-sm font-black text-slate-900 bg-gradient-to-r from-orange-100 to-pink-100 border border-white hover:opacity-95 rounded-xl shadow-xs transition-transform active:scale-95 flex items-center gap-2 cursor-pointer"
                >
            ...
            >
              ...
              <div className="bg-gradient-to-r from-orange-100 via-pink-100 to-orange-50 border-b border-slate-200/50 p-5 text-slate-900 flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-extrabold text-lg tracking-tight">Расширение лимитов ИИSMM</h3>
                </div>
                <button 
              ...
              <div className="p-5 space-y-4">
                {checkoutStep === 'plan' && (
                  <div className="space-y-4">
                    ...
                    <button 
                      ...
                      className="w-full py-2.5 text-sm font-extrabold text-orange-950 bg-gradient-to-r from-orange-100 to-pink-100/90 hover:opacity-95 border border-orange-200/60 rounded-xl shadow-xs transition-all flex items-center justify-center gap-2 cursor-pointer"
                    >
                      <CreditCard className="w-4 h-4 text-orange-800" />
                    ...
                    <div className="p-4 rounded-xl border border-white bg-orange-50/70 text-center space-y-2 shadow-2xs">
                      ...
                      <button 
                        ...
                        className="py-1.5 px-4 text-xs font-black text-slate-900 bg-gradient-to-r from-orange-100 to-pink-100 border border-white hover:from-orange-200 hover:to-pink-200 rounded-lg shadow-2xs w-full cursor-pointer"
                      >
                  ...
                  <form onSubmit={handlePaymentSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2">
                      ...
                      <button 
                        ...
                        type="submit" 
                        className="py-2.5 text-xs text-slate-900 bg-gradient-to-r from-orange-100 to-pink-100 border border-white hover:from-orange-200 hover:to-pink-200 font-black rounded-lg cursor-pointer shadow-2xs"
                      >
                        Оплатить
                      </button>
                    </div>
                ...
                {checkoutStep === 'success' && (
                  <div className="text-center py-6 space-y-4">
                    <div className="inline-flex p-3.5 bg-orange-50 text-orange-600 rounded-full mb-1 border border-white">
                    ...
                    <button 
                      ...
                      className="inline-flex px-5 py-2.5 bg-gradient-to-r from-orange-100 to-pink-100/90 hover:opacity-95 text-orange-950 border border-orange-200 rounded-xl text-xs font-black shadow-2xs cursor-pointer"
                    >
                      Начать постинг
                    </button>
                  </div>
